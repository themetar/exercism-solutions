...
module ZebraPuzzle
  ...
  SAME_HOUSE = ->(ary_a, val_a, ary_b, val_b) { ary_a[ary_b.index(val_b)] == val_a }

  AT_HOUSE = ->(ary, val, index) { ary[index] == val }

  NEXT_TO = ->(ary_a, val_a, ary_b, val_b) { (ary_a.index(val_a) - ary_b.index(val_b)).abs == 1 }
