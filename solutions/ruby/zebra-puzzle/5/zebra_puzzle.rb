...
module ZebraPuzzle
  ...
  CATEGORIES = {
    ...
    nationalities: %i[English Spanish Ukranian Japanese Norwegian].freeze,
    pets: %i[dog snail fox horse zebra].freeze,
    beverages: %i[water coffee tea milk juice].freeze,
    hobbies: %i[dancing painting reading football chess].freeze
  }.freeze

  SAME_HOUSE = ->(collection_a, property_a, collection_b, property_b) { collection_a[collection_b.index(property_b)] == property_a }

  AT_HOUSE = ->(collection, property, index) { collection[index] == property }

  NEXT_TO = ->(collection_a, property_a, collection_b, property_b) { (collection_a.index(property_a) - collection_b.index(property_b)).abs == 1 }
